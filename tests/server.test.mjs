...
test('collection serves catalog, both game entries and assets without exposing source or runtime files',async t=>{
 ...
 const {data:catalog}=await call(app,'/games.json');assert.deepEqual(catalog.map(game=>game.id),['texas-holdem','splendor','abracada-what','aeroplane-chess','buckshot-roulette','steel-arc']);
