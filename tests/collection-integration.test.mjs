...
test('all six Node API adapters preserve rate-limit errors after integration', async t => {
  ...
  for (const game of ['poker', 'splendor', 'abracada', 'aeroplane', 'buckshot', 'steel-arc']) {
...
test('configured Cloudflare migrations support independent rooms for all six games', async t => {
  ...
  for (const game of ['poker', 'splendor', 'abracada', 'aeroplane', 'buckshot', 'steel-arc']) {
    const table=game==='steel-arc'?'steel_arc_rooms':`${game}_rooms`;
