...
`/games/anime-campus/*` 只公开 `web/`。本地与服务端共用纯规则引擎，`/api/anime-campus` 使用独立 `anime-campus-rooms.json`。同时选择与记忆答案通过玩家视角过滤，联机掷骰由服务器产生。Worker 增加 `anime_campus_rooms` / `anime_campus_limits`，需在现有迁移后应用 `0005_anime_campus_rooms.sql`。新游戏尚未随此次本地开发发布到公网；前文四款游戏的公开部署说明仍指既有版本。
