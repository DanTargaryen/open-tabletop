...
function portrait(card) {
  if(card.artIndex>=20 && card.artIndex<40) {
    const n=card.artIndex-20,col=n%5,row=Math.floor(n/5),xs=[0,280,560,841,1122,1402],ys=[0,275,552,810,1122],w=xs[col+1]-xs[col],h=ys[row+1]-ys[row];
    return `<div class="portrait" role="img" aria-label="${escape(card.nameZh+'同人插画')}" style="--atlas:url('./assets/pokemon-atlas.png');background-size:${1402/w*100}% ${1122/h*100}%;background-position:${xs[col]/(1402-w)*100}% ${ys[row]/(1122-h)*100}%"></div>`;
  }
  return `<div class="portrait dex-portrait ${card.bonus}" aria-hidden="true"><span>${String(card.dexId).padStart(3,'0')}</span><small>${escape(card.name.toUpperCase())}</small></div>`;
}
...
function cardHTML(card,{interactive=true}={}) {
  ...
  return `<button class="pokemon-card ${can?'affordable':''}" data-card="${card.id}" aria-label="${escape(card.nameZh+'，'+card.points+' 分，'+labels[card.bonus]+'加成 '+card.bonusAmount)}" ${interactive?'':'disabled'}><div class="card-top"><span class="card-points">${card.points}<small>奖杯</small></span><span class="bonus">${ball(card.bonus)}<small>+${card.bonusAmount}</small></span></div>${portrait(card)}${card.evolveCost?`<div class="evo-cost">进化 ${costs(card.evolveCost)}</div>`:''}<div class="card-bottom"><div class="card-name">${escape(card.nameZh)}<span class="card-id">#${String(card.dexId).padStart(3,'0')}</span></div><div class="costs">${costs(card.cost)}</div></div></button>`;
}
function lobby() {
...
function showPlayer(seat) {
  ...
  document.querySelector('#detail-body').innerHTML=`<span class="eyebrow">PUBLIC TEAM</span><h2>${escape(p.name)}的队伍</h2><p class="fine">${p.points} 奖杯 · ${p.evolved.length} 次进化 · 预留 ${p.reserved.length} 张（内容仅本人可见）</p><div class="public-team">${p.cards.map(c=>cardHTML(c)).join('')||'<p>还没有捕捉到宝可梦。</p>'}</div>`;
  detail.showModal();
}
