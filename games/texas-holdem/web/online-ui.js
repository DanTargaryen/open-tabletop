...
$('tabletopHome')?.addEventListener('click',event=>{if(event.button!==0||event.metaKey||event.ctrlKey||event.shiftKey||event.altKey)return;if(net.session&&net.data?.room.status!=='finished'&&!confirm('返回大厅后，房间会继续运行，超时将自动过牌或弃牌。重新进入好友房可尝试恢复座位。确定返回大厅吗？'))event.preventDefault();});
