...
let state=createGame(),room=null,identity=null,busy=false,pollTimer=null,botTimer=null,boardKey='',lastRoll=0;
...
function ownTurn(){return !busy&&(!online?!state.players[state.turn].isBot:room?.status==='playing'&&room.selfSeat===state.turn);}
...
function render(){
  const actor=state.players[state.turn],color=COLORS[actor.color],key=state.players.map(p=>p.color).join(',');
  document.documentElement.style.setProperty('--team',color.ink);document.documentElement.style.setProperty('--light',color.light);
  if(boardKey!==key){drawBoard($('board'),state.players);boardKey=key;}
  const active=!online||room?.status==='playing',mine=ownTurn();
  updatePlanes($('board'),state,active&&mine&&state.phase==='move');
  $('turn-panel').hidden=!active||state.phase==='finished';
  $('active-icon').innerHTML=planeIcon;$('active-color').textContent=`${color.name}机队 · ${actor.name}`;
  $('turn-title').textContent=state.phase==='finished'?'全员抵达':mine?state.phase==='move'?'选择一架飞机':'轮到你了':actor.isBot?'正在思考…':'等待好友…';
  $('roll-button').disabled=!active||!mine||state.phase!=='roll';
  $('roll-button').textContent=busy?'正在同步…':state.phase==='move'?'点选飞机':mine?'掷骰子':'等待行动';
  const hint=state.phase==='move'?mine?`掷出 ${state.die} 点 · 点击高亮飞机或下方按钮`:`${actor.name} 正在选择飞机`:mine?'掷出 6 点即可起飞，也可前进 6 格':`${actor.name} 的回合${online?' · 超时将由 AI 代打':''}`;
  $('turn-hint').textContent=hint;
  const patterns={1:[4],2:[0,8],3:[0,4,8],4:[0,2,6,8],5:[0,2,4,6,8],6:[0,2,3,5,6,8]};
  $('die').querySelectorAll('i').forEach((el,i)=>{el.style.opacity=(patterns[state.die||6]).includes(i)?'1':'0';});
  $('die').setAttribute('aria-label',state.die?`骰子 ${state.die} 点`:'尚未掷骰');
  if(state.rolls!==lastRoll){lastRoll=state.rolls;$('die').classList.remove('rolling');void $('die').offsetWidth;$('die').classList.add('rolling');}
  $('move-choices').innerHTML=active&&mine?legalPieces(state).map(piece=>{
    const from=actor.planes[piece],to=moveRoute(actor.color,from,state.die).stops.at(-1);
  ...
  $('roster').innerHTML=state.players.map(player=>{
    ...
    const label=online?(room?.roster.some(m=>m.seat===player.id)?(player.id===room.selfSeat?'你':'好友'):'本地 AI'):player.isBot?'本地 AI':state.mode==='local'?'同屏玩家':'你';
    return `<div class="roster-row ${state.turn===player.id?'current':''}" style="--team:${c.ink};--light:${c.light}"><span class="roster-plane">${planeIcon}</span><span class="roster-name">${escape(player.name)}<small>${c.name} · ${label}</small></span><span class="progress-count">${count} / 4</span><span class="progress-dots" aria-hidden="true">${Array.from({length:4},(_,i)=>`<i class="${i<count?'done':''}"></i>`).join('')}</span></div>`;
  }).join('');
  $('event-log').innerHTML=state.events.length?[...state.events].reverse().slice(0,6).map(e=>`<li>${escape(e.text)}</li>`).join(''):'<li>飞机就位，把快乐飞得更远。</li>';
  $('result').hidden=state.phase!=='finished';
  if(state.phase==='finished')$('winner-title').textContent=`${state.players[state.winner].name} 赢得本局`;
  $('play-again').hidden=online;
  $('mode-label').textContent=online?`好友房${room?' · '+room.code:''}`:state.mode==='local'?`${state.players.length} 人 · 同屏轮流操作`:'单人 · 本地 AI';
  soundEvents(state);renderRoom();scheduleBot();
}

function scheduleBot(){
  clearTimeout(botTimer);
  if(online||disposed||busy||document.hidden||$('setup-dialog').open||$('rules-dialog').open||state.phase==='finished'||!state.players[state.turn].isBot)return;
...
async function action(type,piece){
  ...
  if(online){
    ...
    try{type==='roll'?rollDice(state):movePiece(state,piece);persist();render();}catch(e){error(e.message);}
  }
}
...
function applySnapshot(data,{initial=false}={}){
  ...
  if(initial||document.hidden||state.eventId<previousEventId)resetAudio(state);
  if(pendingAction&&room.version>pendingAction.version)pendingAction=null;
  $('connection').textContent='已连接';error();render();
}
function clearIdentity(){identity=null;room=null;pendingAction=null;networkGeneration++;save(IDENTITY,null);resetAudio(state);state=createGame();render();}
...
async function poll(){
  ...
  try{const data=await api(`/rooms/${identity.code}`,{token:identity.token});if(generation===networkGeneration)applySnapshot(data,{initial:!room});}catch(e){if(generation===networkGeneration)handleFailure(e);}
...
function renderRoom(){
  ...
  $('room-waiting').hidden=!room||room.status==='playing';
...
$('setup-form').addEventListener('submit',event=>{event.preventDefault();state=createGame({playerCount:Number($('player-count').value),mode:$('local-mode').value});resetAudio(state);lastRoll=0;error();persist();$('setup-dialog').close();render();});
...
document.addEventListener('visibilitychange',()=>{resetAudio(state);if(!document.hidden){online?poll():scheduleBot();}else clearTimeout(botTimer);});
window.addEventListener('pagehide',()=>{disposed=true;clearTimeout(botTimer);clearTimeout(pollTimer);});
...
resetAudio(state);lastRoll=state.rolls;soundButton();
